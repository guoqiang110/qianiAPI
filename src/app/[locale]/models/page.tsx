...
const badgeColors: Record<string, string> = {
  ...
  "待配置":"bg-slate-100 text-slate-500 ring-slate-200/50",
...
export default function ModelsPage() {
  ...
  const models = allModels.filter(m => m.category === activeCat);
    ...
    <div className="flex-1">
      <section className="border-b border-sky-100 bg-[radial-gradient(circle_at_top_left,_rgba(37,99,235,0.16),_transparent_30%),linear-gradient(180deg,_rgba(255,255,255,0.98),_rgba(239,246,255,0.90))]">
        <div className="mx-auto max-w-7xl px-6 py-12 lg:py-16">
          ...
          <p className="mt-3 max-w-2xl text-lg leading-8 text-slate-600">覆盖图片生成、图片处理、语言模型等品类，通过统一 API 网关接入。价格与后台配置实时同步。</p>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-7xl px-6 py-8 space-y-8">
        <div className="flex flex-wrap gap-3">
          {CATEGORIES.map(c => (
            <button key={c.id} onClick={() => setActiveCat(c.id)} className={`inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition-all ${activeCat===c.id?"bg-slate-950 text-white shadow-[0_14px_32px_rgba(15,23,42,0.20)]":"border border-slate-200 bg-white text-slate-600 hover:border-sky-200 hover:bg-sky-50 hover:text-slate-950"}`}>
